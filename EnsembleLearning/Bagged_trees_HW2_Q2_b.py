...
class BagTree():
    ...
    def train_model(self,train,test,Numeric):
        ...
        #plt.figure(2)
        #plt.plot(range(len(self.train_accu)),self.train_accu,range(len(self.test_accu)),self.test_accu,'Training set','Testing set','Prediction Error','Iterations','Error')
        #print(self.medians)
...
'''Main function'''
def main():
    Train=pd.read_csv(r'D:/GoogleDirve/phd/5/class/HW/HW2/train.csv',header=None)
    Test=pd.read_csv('D:/GoogleDirve/phd/5/class/HW/HW2/test.csv',header=None)
    ...
    BT=BagTree(tree_no=500,sampling_rate=0.005)
